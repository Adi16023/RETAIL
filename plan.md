...
**AryaChat screen matches youkti's conversation shell (Sept 7).** The app sidebar is the chat sidebar: New chat (replacing the AryaChat tab), search, and grouped conversations. The main pane is only the hero or thread plus the composer. A conversation file is created only on the first send.

**AryaChat lifted to the whole book (Sept 7).** At the user's request the chat now sits on top of everything: first in the nav, no account to pick, no pre-written questions. The model gets a small account index (id, name, region, verdict-if-any) plus the brief of the account in focus, and a sixteen-tool catalogue it calls MCP-style — find an account by name, the book as a table, an account's brief, a comparison digest for 2–8 accounts, and per-account orders, products, category mix, monthly table, KPIs, ruled-out checks, priced options and the five investigator drill-downs (all widened with `account_id`). Focus is sticky per chat (the last single-account tool call sets it; a comparison moves nothing) and is seeded from the account open elsewhere in the app. Chats are keyed by the whole file's fingerprint. Verified live on `claude-sonnet-5`: "which accounts should I call first" → the account book, a ranked table, no unsourced figures; "how often does Northgate order" → orders + KPIs, focus set to ACC-101, and the figure check caught the model's own rounded ranges ("Rs 250,000–290,000"); "compare it with Harbor" → resolved Harbor from the index, one `compare_accounts` call, focus kept. 362 tests. Known limit: ~16k input tokens a turn on Anthropic is over the Groq free tier's per-minute cap for book-level questions.

**Lifetime value (Sept 8) — a third nav page, CLTV per account.** `src/pipeline/lifetime.py`: BG/NBD fitted on the pooled book (SciPy, no new dependency) for the probability an account is still active and its expected orders over 12 and 24 months; converted to expected active months and multiplied by margin per month, baseline vs recent, discounted at 10% a year. The page leads with the gap — value at risk, the leak's lifetime cost — and ranks the book by it when no account is open. Two measured decisions: value per **month** (per order reported a fifth of ACC-102 at risk purely because it now splits orders; per month it is flat) and **margin** (revenue would clear ACC-101 and ACC-107). The block is an underscore key on the pack so the investigator never sees it. Backtest `scripts/check_lifetime_model.py` (now `lifetime.backtest`, also quoted on the page): fit on history minus 6 months, predict the 6 — 10.5% book-level order error, PASS. ACC-109 is insufficient_history. The page grew to five elements and was then trimmed back to three at the user's request the same day: the value-per-month change tile (replacing "still buying", which is 100% on a book where nobody has lapsed), the two-path chart and the horizon table stay, and a category and a product breakdown of the same projection were added (parts sum to the account; line losses exceed the net because grown lines offset them); the dated reasons, the priced moves' lifetime effect and the rank/backtest lines came off the page (their functions remain in the pipeline, tested). 12 tests.

**AryaChat rebuilt on the ARYA MCP chat loop (Sept 6, later).** At the user's request the chat now works like `youkti-arya/core/mcp_chat`: prose answers streamed as they are written, a `tool_start`/`tool_end` event per drill-down (shown as chips that flip from running to done and open to the result preview), `<follow_up>` next-question chips parsed out with the same stream-safe filter (tap sends verbatim; ephemeral), a `retry` event when a product/seasonal/pricing draft is refused for skipping its tool, a capped tool result with a visible marker, summed token usage, a truncated-not-errored step ceiling, and a delete button on the chat list. The `submit_answer` contract is gone; in its place `unsourced_figures` checks every number in the finished answer against the brief, this turn's tool results and the earlier exchange, and the screen flags what it cannot find — the first test written against it caught a "32.2%" where the brief said 32.1. Streaming rides the injected client's `messages.stream` (Anthropic native; `_GroqStream` in the shim assembles chunked tool-call fragments; the test fake scripts it) and falls back to `create` when absent. Not ported, on purpose: confirm gate, result hardening, web search, Postgres, idempotency — no outward actions, no third-party text, no server here. 359 tests; a headless `AppTest` run of the page with a scripted client renders the streamed answer, the tool expander, the flagged figure and the chips.

**AryaChat (Sept 6) — a conversational fourth agent.** A manager asks about one account; `src/pipeline/aryachat.py` answers from an account brief (~3k chars: verdict, money, six signals in plain words, KPI tiles, stopped categories, dated events) plus the five drill-down tools and four chat-only tools (monthly table, KPIs, ruled-out checks, priced options), with a `submit_answer` contract (≤80 words, figures cited, what the data cannot answer) and a per-answer log of tools used. Context is per chat: `src/ui/chatstore.py` stores one JSON per chat under `.cache/aryachat/` (no database), replays the last 6 turns verbatim, and compacts older turns into a summary via one `submit_summary` call — automatically past 12 turns, or on demand. New page "AryaChat" in app.py; 17 new tests (341 total). Live on Groq: first answer in ~2s from the brief alone, figures matching the dashboard tiles after the rounding-order fix.

**Sept 6 — agent output rewritten for the screen; first live Anthropic scoring.** The three prompts (agent, compare, decide) were compressed and given per-field length limits and a numbers-for-a-manager rule (₹88,810, 32.2%, April 2026, "down 12 points"; never a p-value, raw ratio, "pp" or field name), because their output is read on a Streamlit page, not in a document. The verdict panel lost its six tabs and its second-opinion section: banner, money, narrative, "Why" as plain sentences, and one collapsed "What changed, and when" timeline dropdown (the ruled-out checks, dimension table and sufficiency block were dropped from this screen at the user's request; the PDF and Detect page still carry them); the classifier's probability for the AI's outcome sits in brackets on the confidence line, and it shows no actions — the decision step owns those. **Scores**: Groq `gpt-oss-120b` 18/18 twice on the compressed prompt (the daily free-tier allowance then ran out — 200k tokens/day — so the final wording is unscored on Groq); Anthropic `claude-sonnet-5` 18/18 three times on successive wordings, the last being the plain-text one. Two Anthropic-only failures were found and fixed on the way, neither a verdict: (1) Sonnet 5 runs adaptive thinking by default and it is billed against `max_tokens`, so a 4,096 ceiling truncated the tool call on ACC-106 — the agent now asks for 16,000 and the Groq shim clamps to `GROQ_MAX_OUTPUT_TOKENS` (4,096), keeping the provider difference inside the shim; (2) a reply cut off at `max_tokens` could return a partial `submit_verdict` that was accepted as complete — the loop now treats `max_tokens` as "not a verdict" (replays text-only content with a nudge, drops a cut-off tool_use) and re-asks when a submit lacks a field the report indexes. A third, cosmetic: in 2 of 18 Sonnet verdicts (both healthy, both with an empty action list) the narrative string ended with the model's own tool markup (`</narrative><parameter name="recommended_actions">[]`) — `agent._strip_tool_markup` now cuts a string at the first such tag before the verdict leaves Stage 4.
