...
- [generate_data.py](generate_data.py): order frequency raised to 8–15/month (sparse_short_history archetype deliberately kept low — that's its whole point).
...
- [x] Live-failure hardening — timeout/retry is the Anthropic SDK's default (2 retries w/ backoff on 429/5xx/connection errors, not reimplemented); a most-specific-first exception chain in `app.py` catches auth/rate-limit/connection/status errors from a real API call and shows a clear message while still displaying the (already-computed, deterministic) Stage 1-3 evidence — verified live by uploading a real file with no API key present: the app showed the error and kept the evidence visible rather than crashing. Offline fallback: [demo_cache/](demo_cache/) holds pre-computed reports (scripted verdict, clearly labeled as not a live LLM call) for the flagship structural archetype and a healthy account, generated by [scripts/generate_demo_cache.py](scripts/generate_demo_cache.py) and selectable in the UI's "View offline demo" mode. No hard-coded absolute paths — everything relative to `app.py` or from the uploaded file object.
