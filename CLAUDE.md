...
1. **What the data says** — [ui/dashboard.py](src/ui/dashboard.py). Status strip, KPI tiles, six tabs. Fully deterministic, so clicking through all 18 accounts costs nothing. This is also exactly what Stage 4 is shown. The account book (the table Detect opens on, [ui/accounts.py](src/ui/accounts.py)) shows six columns since Sept 7: Account, Name, Region, Revenue signal, **Revenue leakage** (Detected / Not detected / Deferred / Not analysed from the cached report, at the user's request) and **Probability of leakage** — the classifier's P(FLAG) for **every** row, whatever the AI concluded, so the column always reads one way and a Healthy row with a high number is a visible disagreement. It was "Statistical model confidence" (the probability of whichever outcome the AI reached) for one day; that flipped direction row by row and was read as a leak score anyway. A deferred account, or one whose classifier leaning is "defer", shows a dash with "not enough history" on hover, never a reassuring 1%. Manager, the AI's own confidence level and the per-dimension figures were dropped from the table at the user's request; they stay in the frame for search and filters, and the AI's level is on the verdict banner. Since Sept 7 the page opens with the **verdict at a glance** (`render_verdict_header` in [ui/verdict.py](src/ui/verdict.py): banner, leak dimensions, the four money tiles) whenever a verdict is cached for this account, model and fingerprint; otherwise a caption and an "Analyse with AI" button run the investigation in place and save it, so the banner appears without leaving the page. The reasoning, timeline and PDF stay on Investigate.
