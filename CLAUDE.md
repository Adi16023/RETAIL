...
1. **What the data says** — [ui/dashboard.py](src/ui/dashboard.py). Status strip, KPI tiles, six tabs. Fully deterministic, so clicking through all 18 accounts costs nothing. This is also exactly what Stage 4 is shown. Since Sept 7 the page opens with the **verdict at a glance** (`render_verdict_header` in [ui/verdict.py](src/ui/verdict.py): banner, leak dimensions, the four money tiles) whenever a verdict is cached for this account, model and fingerprint; otherwise a caption and an "Analyse with AI" button run the investigation in place and save it, so the banner appears without leaving the page. The reasoning, timeline and PDF stay on Investigate.
2. **What the AI concludes** — the one LLM call, then [ui/verdict.py](src/ui/verdict.py). One screen, no tabs: banner, money, narrative, then "Why" as plain sentences, and a single collapsed "What changed, and when" dropdown holding the dated timeline (the same rows the PDF prints). Nothing else: no ruled-out checks, no dimension table, no data-sufficiency block — those live on the Detect page and in the PDF. It shows no actions — the decision step owns "what to do", and a second list here made the screen say two things. All three prompts carry a numbers-for-a-manager rule (₹88,810, 32.2%, April 2026, "down 12 points"; never a p-value, raw ratio, "pp" or field name). The classifier has no section of its own: the banner carries one labelled line per source — "AI agent — High confidence" (the judge's own level) and "Statistical model — 91% confidence score" (the witness's probability for the outcome the AI reached, so a low figure is an honest disagreement) — followed by the priority line, which comes from neither. The three agent prompts carry explicit length limits per field (narrative ≤3 sentences, facts ≤5 × 25 words, actions ≤4 × 20 words, and so on) because the output is read on screen, not in a document.
